...
describe("plugin.codex", () => {
  ...
  test("uses Codex context limits for OAuth GPT models", async () => {
    ...
    const directLimit = { context: 1_050_000, input: 922_000, output: 128_000 }
    const provider = {
      models: Object.fromEntries(
        ["gpt-5.4", "gpt-5.5", "gpt-5.6", "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"].map((id) => [
          id,
          { id, api: { id }, limit: directLimit, cost: {} },
        ]),
      ),
    }
    ...
    expect(models["gpt-5.4"]?.limit).toEqual(directLimit)
    ...
    expect(provider.models["gpt-5.6"]?.limit).toBe(directLimit)
    expect(await hooks.provider!.models!(provider as never, {} as never)).toBe(provider.models as never)
